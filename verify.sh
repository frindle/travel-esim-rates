#!/usr/bin/env bash
# verify for: esim-global-s2-picker-exclude-global
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('build.py').read())"; then
  echo "  ok: build.py parses"
else
  echo "  FAIL: build.py does not parse"; fails=$((fails+1))
fi
